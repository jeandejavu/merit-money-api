...
import { CheckFieldError, MissingParamError } from '@/presentation/errors';
...
const makeSut = (): SutTypes => {
  ...
  const sut = new AuthenticationController(authenticationSpy, validationSpy);
  return {
    sut,
    authenticationSpy,
    validationSpy,
...
describe('Authentication Controller', () => {
  ...
  test('Should return 401 if invalid credentials are provided', async () => {
    ...
    jest.spyOn(authenticationSpy, 'auth').mockImplementationOnce((): never => {
      throw new CheckFieldError('any_error');
    });
